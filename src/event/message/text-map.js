...
dotenv.config();
const richMenuId = process.env.RICH_MENU_BUSINESS_ID;
const getUserProfile = (event, client) =>
  client.getProfile(event.source.userId);
const changeRichMenu = (event, client) =>
  client.linkRichMenuToUser(event.source.userId, richMenuId);
...
export const messageMap = {
  ...
  企業: async (event, appContext) => {
    // ユーザーのプロフィール情報を取得
    await changeRichMenu(event, appContext.lineClient);
    // 返信するメッセージを作成
    return {
      type: "text",
      text: "リッチメニューを「企業用」に変更しました",
    };
  },
};
